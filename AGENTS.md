...
- `cmd/openauth`: primary CLI entrypoint with `migrate up [steps]`, `migrate down <steps>`, `migrate force <version>`, and `migrate goto <version>` subcommands.
...
- v0 CLI migration runner uses `golang-migrate` with the PostgreSQL `pgx/v5` database backend for `cmd/openauth migrate up [steps]`, `cmd/openauth migrate down <steps>`, `cmd/openauth migrate force <version>`, and `cmd/openauth migrate goto <version>`, with file-based migration sources under adapter-specific directories.
